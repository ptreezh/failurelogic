...
def check_deployment_status():
    ...
    try:
        # 检查服务状态
        result = subprocess.run(['railway', 'status'], capture_output=True, text=True, encoding='utf-8')
        print(f"Railway状态:\n{result.stdout}")
        if result.stderr:
...
def check_service_details():
    ...
    try:
        # 获取服务列表
        result = subprocess.run(['railway', 'service', 'list'], capture_output=True, text=True, encoding='utf-8')
        print(f"服务列表:\n{result.stdout}")
    except Exception as e:
        print(f"❌ 无法获取服务列表: {e}")
...
def analyze_deployment_logs():
    ...
    try:
        result = subprocess.run(['railway', 'logs'], capture_output=True, text=True, encoding='utf-8')
        logs = result.stdout
