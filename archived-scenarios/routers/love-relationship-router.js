/**
 * Love Relationship Scenario Router
 * 恋爱关系场景路由器
 * 处理所有恋爱关系认知训练场景 (love-relationship-001 ~ love-relationship-003)
 */

(function(global) {
  'use strict';

  class LoveRelationshipRouter {
    constructor(container, apiClient) {
      this.container = container;
      this.apiClient = apiClient;
      this.scenarioData = null;
      this.currentCase = null;
      this.currentStep = 0;
      this.decisions = [];
      this.startTime = Date.now();
    }

    async initialize(scenarioId) {
      try {
        this.scenarioData = await this.loadScenarioData();
        this.currentCase = this.scenarioData.find(c => c.scenarioId === scenarioId) || this.scenarioData[0];
        this.currentStep = 0;
        this.decisions = [];
        this.renderIntro();
      } catch (error) {
        Logger?.error('LoveRelationshipRouter', 'Initialization failed:', error);
        this.showError('恋爱关系场景加载失败，请刷新页面重试');
      }
    }

    async loadScenarioData() {
      if (this.apiClient) {
        try {
          const data = await this.apiClient.getLoveRelationshipScenarios();
          if (data && data.length > 0) return data;
        } catch (e) {
          Logger?.warn('LoveRelationshipRouter', 'API加载失败，使用本地数据');
        }
      }

      return this.getDefaultScenarioData();
    }

    getDefaultScenarioData() {
      return [
        {
          scenarioId: 'love-relationship-001',
          title: '恋爱关系认知训练 - 初识阶段',
          description: '模拟恋爱关系初识阶段的决策过程，测试用户在情感决策中的思维模式',
          steps: [
            {
              step: 1,
              situation: '你在一个朋友聚会上认识了一个很有吸引力的人，感觉彼此很聊得来。聚会结束后，对方提出交换联系方式。此时你会如何回应？',
              options: [
                '立即交换联系方式，并提议明天见面',
                '交换联系方式，但保持适当距离，慢慢了解',
                '礼貌拒绝，觉得还不够了解对方',
                '只交换社交媒体账号，先线上了解'
              ],
              explanation: '不同的选择反映了用户在亲密关系中的风险偏好和期望管理方式。立即推进可能反映理想化倾向，而过度谨慎可能反映回避型依恋。'
            },
            {
              step: 2,
              situation: '经过几周的交流，你们决定正式约会。第一次约会后，对方对你表达了强烈的兴趣，希望尽快确立关系。你发现自己对这段关系也有一些期待，但内心仍有疑虑。',
              options: [
                '同意确立关系，珍惜这份感情',
                '表达需要更多时间了解，建议继续约会观察',
                '坦诚告知自己的疑虑，讨论彼此的期望',
                '开始考虑其他选择，不急于确定关系'
              ],
              explanation: '此决策测试用户如何平衡情感冲动与理性判断，以及在关系中的沟通能力。'
            },
            {
              step: 3,
              situation: '关系确立后几个月，你注意到一些之前未曾察觉的行为模式：对方有时会忽视你的感受，或者在重要决定上不征求你的意见。你开始质疑这段关系是否健康。',
              options: [
                '直接与对方沟通这些问题，寻求改变',
                '暂时忽略这些问题，希望它们会自然改善',
                '向朋友或家人寻求建议',
                '开始考虑结束这段关系'
              ],
              explanation: '此决策测试用户在面对关系问题时的应对策略，以及自我价值的认知。'
            }
          ],
          analysis: {
            purpose: '测试用户在恋爱关系中的决策模式和情感认知能力',
            decisionPatternsTested: ['理想化倾向', '确认倾向', '沉没成本效应', '可得性倾向'],
            learningObjectives: ['识别恋爱关系中的决策模式', '学会平衡情感与理性', '建立健康的沟通模式', '认识并维护个人边界']
          }
        },
        {
          scenarioId: 'love-relationship-002',
          title: '恋爱关系认知训练 - 长期关系维护',
          description: '模拟长期恋爱关系中的挑战与决策，测试用户在维持关系中的认知能力',
          steps: [
            {
              step: 1,
              situation: '你和伴侣在一起两年了，关系稳定。最近你们开始讨论未来规划，包括居住地、职业发展和是否要孩子等问题。你们在某些问题上有分歧。',
              options: [
                '坚持自己的观点，认为自己的想法更合理',
                '完全迁就伴侣的想法，避免冲突',
                '寻求折中方案，平衡双方需求',
                '推迟讨论，认为时间会解决分歧'
              ],
              explanation: '此决策测试用户在长期关系中的沟通和妥协能力，以及对冲突的态度。'
            },
            {
              step: 2,
              situation: '在一次重要争吵后，你们进入了冷战期。你感到受伤和失望，但同时也想念对方。此时你会如何处理？',
              options: [
                '主动道歉并尝试和解，即使觉得自己没错',
                '等待对方先主动联系',
                '通过共同朋友传达和解意愿',
                '冷静一段时间后再沟通，准备充分的谈话'
              ],
              explanation: '此决策测试用户在冲突后的情感调节能力和修复关系的意愿。'
            },
            {
              step: 3,
              situation: '你们的关系经历了起伏，现在面临一个重要决定：是否同居或结婚。双方家庭也对此有不同的期望和意见。',
              options: [
                '按照原计划推进，不考虑外界意见',
                '暂停决定，花更多时间评估关系',
                '寻求专业咨询师的建议',
                '根据家庭意见调整计划'
              ],
              explanation: '此决策测试用户在重大人生决定中的独立思考能力，以及处理外部压力的能力。'
            }
          ],
          analysis: {
            purpose: '测试用户在长期关系维护中的决策模式和应对能力',
            decisionPatternsTested: ['承诺升级倾向', '社会期望倾向', '损失规避', '群体思维'],
            learningObjectives: ['学会处理长期关系中的冲突', '平衡个人需求与关系需求', '做出明智的人生决定', '建立健康的依恋模式']
          }
        },
        {
          scenarioId: 'love-relationship-003',
          title: '恋爱关系认知训练 - 沟通与理解',
          description: '专注于恋爱关系中的沟通技巧和相互理解，测试用户的情感智商',
          steps: [
            {
              step: 1,
              situation: '你的伴侣最近工作压力很大，回家后显得疲惫和烦躁。当你试图关心时，对方显得不耐烦甚至有些冷漠。',
              options: [
                '直接询问对方发生了什么，要求详细解释',
                '给予空间，暂时不打扰',
                '表达自己的感受，说明关心对方',
                '准备一些放松的活动来缓解气氛'
              ],
              explanation: '此决策测试用户在伴侣情绪低落时的支持方式和沟通技巧。'
            },
            {
              step: 2,
              situation: '你发现伴侣对你隐瞒了一些事情，虽然不是原则性问题，但让你感到被欺骗。你不确定是否应该直接询问。',
              options: [
                '直接询问，强调诚实的重要性',
                '等待合适的时机再提起',
                '观察是否会自行解决',
                '反思自己是否也有类似行为'
              ],
              explanation: '此决策测试用户在面对信任问题时的处理方式和沟通策略。'
            }
          ],
          analysis: {
            purpose: '测试用户在恋爱关系中的沟通技巧和情感理解能力',
            decisionPatternsTested: ['个人化倾向', '读心术倾向', '情绪推理', '假设陈述'],
            learningObjectives: ['提高情感识别能力', '学习有效的沟通技巧', '建立信任和安全感', '处理关系中的误解']
          }
        }
      ];
    }

    renderIntro() {
      if (!this.currentCase) return;

      const html = `
        <div class="scenario-intro relationship-intro compact-start-page">
          <h1>${this.currentCase.title}</h1>
          <p class="description">${this.currentCase.description}</p>
          <div class="case-meta compact-stats-grid">
            <span class="case-type">💕 恋爱关系训练</span>
            <span class="case-steps">📋 ${this.currentCase.steps.length} 个情境</span>
          </div>
          <div class="instructions compact-situation">
            <h3>场景说明</h3>
            <p>您将面对恋爱关系中的真实情境挑战。每个选择都反映了特定的认知模式，请根据您的真实想法做出选择。</p>
            <p class="warning">没有"正确"答案，但每个选择都有其心理含义。完成后的分析将帮助您更好地了解自己的情感决策模式。</p>
          </div>
          <button class="start-btn" onclick="window.loveRelationshipRouter.startScenario()">
            开始体验
          </button>
        </div>
      `;

      HTMLSanitizer?.setInnerHTML(this.container, html);
    }

    startScenario() {
      this.currentStep = 0;
      this.decisions = [];
      this.renderCurrentStep();
    }

    renderCurrentStep() {
      if (!this.currentCase) return;

      if (this.currentStep >= this.currentCase.steps.length) {
        this.renderConclusion();
        return;
      }

      const step = this.currentCase.steps[this.currentStep];
      const html = `
        <div class="scenario-round relationship-round compact-page-header">
          <div class="round-header compact-stats-grid">
            <span class="step-indicator">情境 ${this.currentStep + 1} / ${this.currentCase.steps.length}</span>
          </div>
          <div class="round-context compact-situation">
            <h2>${step.situation}</h2>
          </div>
          <div class="decision-panel compact-actions">
            <h3>您的选择：</h3>
            <div class="options-list compact-options-grid">
              ${step.options.map((opt, idx) => `
                <button class="option-btn relationship-option compact-option-card" 
                        onclick="window.loveRelationshipRouter.makeDecision(${idx})">
                  ${opt}
                </button>
              `).join('')}
            </div>
          </div>
        </div>
      `;

      HTMLSanitizer?.setInnerHTML(this.container, html);
    }

    makeDecision(optionIndex) {
      const step = this.currentCase.steps[this.currentStep];
      this.decisions.push({
        step: this.currentStep + 1,
        situation: step.situation,
        selectedOption: step.options[optionIndex],
        optionIndex: optionIndex,
        explanation: step.explanation
      });

      this.currentStep++;
      this.renderCurrentStep();
    }

    renderConclusion() {
      if (!this.currentCase) return;

      const html = `
        <div class="scenario-conclusion relationship-conclusion compact-start-page">
          <h1>情境分析：${this.currentCase.title}</h1>
          
          <div class="decisions-review-section compact-situation">
            <h2>🎯 您的决策路径</h2>
            <div class="decisions-list">
              ${this.decisions.map(d => `
                <div class="decision-item">
                  <span class="decision-step">情境 ${d.step}</span>
                  <span class="decision-text">${d.selectedOption}</span>
                  <span class="decision-hint">${d.explanation}</span>
                </div>
              `).join('')}
            </div>
          </div>

          <div class="analysis-section compact-situation">
            <h2>📊 认知模式分析</h2>
            <p><strong>测试目的：</strong>${this.currentCase.analysis.purpose}</p>
            <div class="patterns-tested">
              <h4>检测的决策模式：</h4>
              <div class="patterns-list">
                ${this.currentCase.analysis.decisionPatternsTested.map(pattern => `
                  <span class="pattern-tag">${pattern}</span>
                `).join('')}
              </div>
            </div>
          </div>

          <div class="learning-section compact-situation">
            <h2>📚 学习要点</h2>
            <ul>
              ${this.currentCase.analysis.learningObjectives.map(obj => `<li>${obj}</li>`).join('')}
            </ul>
          </div>

          <div class="action-buttons compact-actions">
            <button onclick="window.loveRelationshipRouter.restart()">重新体验</button>
            <button onclick="window.loveRelationshipRouter.loadNextCase()">下一个情境</button>
            <button onclick="NavigationManager.navigateTo('scenarios')">返回场景列表</button>
          </div>
        </div>
      `;

      HTMLSanitizer?.setInnerHTML(this.container, html);
    }

    async loadNextCase() {
      if (!this.scenarioData || this.scenarioData.length === 0) return;

      const currentIndex = this.scenarioData.findIndex(c => c.scenarioId === this.currentCase?.scenarioId);
      const nextIndex = (currentIndex + 1) % this.scenarioData.length;
      this.currentCase = this.scenarioData[nextIndex];
      this.currentStep = 0;
      this.decisions = [];
      this.renderIntro();
    }

    restart() {
      this.currentStep = 0;
      this.decisions = [];
      this.startScenario();
    }

    showError(message) {
      HTMLSanitizer?.setInnerHTML(this.container, `
        <div class="error-message">
          <p>${message}</p>
          <button onclick="window.location.reload()">刷新页面</button>
        </div>
      `);
    }
  }

  global.LoveRelationshipRouter = LoveRelationshipRouter;
  global.loveRelationshipRouter = null;

  global.initLoveRelationship = function(containerId, apiClient) {
    const container = document.getElementById(containerId);
    if (!container) {
      Logger?.error('Container not found:', containerId);
      return null;
    }

    global.loveRelationshipRouter = new LoveRelationshipRouter(container, apiClient);
    return global.loveRelationshipRouter;
  };

})(typeof window !== 'undefined' ? window : this);
