...
async def final_validation():
    ...
    print("1. 验证后端API服务...")
    try:
        response = requests.get("http://localhost:8003/scenarios/", timeout=10)
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                if scenario_cards > 0:
                    ...
                    # 可能需要点击导航到场景页面
                    nav_elements = await page.locator("text=场景|Scenarios|认知场景").first()
                    if await nav_elements.count() > 0:
                        await nav_elements.click()
                        await page.wait_for_timeout(2000)
                        scenario_cards = await page.locator(".scenario-card").count()
                        if scenario_cards > 0:
                            print(f"   ✓ 导航后找到 {scenario_cards} 个场景卡片")
                            success_count += 1
                        else:
                            print("   ⚠ 导航后仍未找到场景卡片")
                    else:
        ...
        # 保持浏览器打开以便用户交互
        print(f"\n   浏览器将保持打开状态供您体验。交互测试成功率: {success_count}/{total_count}")
        
        # 不关闭浏览器，让用户可以交互测试
        input("按Enter键继续...")
        await browser.close()
        
        success_data = (success_count, total_count)
        return success_data
    ...
    if success_data[0] == success_data[1] and api_success == len(api_tests):
        ...
        print(f"⚠ 部分验证未通过 - 交互功能: {success_data[0]}/{success_data[1]}, API功能: {api_success}/{len(api_tests)}")
        return False
