...
async def main():
    async with async_playwright() as p:
        ...
        # 等待场景列表加载 - 使用更长的超时时间
        try:
            await page.wait_for_selector('#scenarios-list .card', timeout=15000)
            print("✓ 场景列表已加载")
        except:
            print("✗ 场景列表加载超时")
            # 再次检查是否有场景卡片
            cards = await page.locator('.card').count()
            print(f"当前找到 {cards} 个卡片")
            if cards == 0:
                # 如果还是没有，则等待更长时间
                await page.wait_for_timeout(10000)
                cards = await page.locator('.card').count()
                print(f"延长等待后找到 {cards} 个卡片")
        ...
        for scenario in scenarios:
            # 返回场景列表页
            if scenario != scenarios[0]:  # 第一个场景不需要返回
                try:
                    # 点击返回场景列表按钮
                    await page.locator('text="返回场景列表"').click(timeout=5000)
                    await page.wait_for_timeout(2000)
                except:
                    # 如果没有返回按钮，说明还在场景列表页面
                    print("仍在场景列表页面")
