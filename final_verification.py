...
def final_verification():
    ...
    with sync_playwright() as p:
        # 启动浏览器
        browser = p.chromium.launch(headless=False)  # 保持可见以便查看
        page = browser.new_page()
        
        # 设置视口大小
        page.set_viewport_size({"width": 1280, "height": 720})
        
        print("🌐 访问修复后的应用: http://localhost:8080")
        ...
        if scenario_link.count() > 0:
            ...
            if card_count > 0:
                ...
                if start_buttons.count() > 0:
                    ...
                    # 检查是否进入了场景
                    scenario_content = page.locator('.scenario-container')
                    if scenario_content.count() > 0:
                        print("✅ 已成功进入场景")
