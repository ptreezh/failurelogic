...
class ScenarioNavigationAgent:
    ...
    async def run_test(self, browser):
        logger.info(f"[{self.name}] 开始测试场景页面导航和难度选择器功能")
        ...
        try:
            ...
            if scenario_nav_button:
                ...
                if scenario_page_active:
                    logger.info(f"[{self.name}] ✅ 成功导航到场景页面")
                    self.results['navigation'] = True
                else:
                    logger.warning(f"[{self.name}] ⚠️ 未能确认到达场景页面")
                    self.results['navigation'] = False
            else:
                logger.error(f"[{self.name}] ❌ 未找到场景导航按钮")
                self.results['navigation'] = False
            ...
            if difficulty_selector:
                ...
                logger.info(f"[{self.name}] 当前难度: {current_difficulty}")
                ...
                if new_difficulty == "intermediate":
                    logger.info(f"[{self.name}] ✅ 成功切换到中级难度")
                    ...
                    if new_difficulty == "advanced":
                        logger.info(f"[{self.name}] ✅ 成功切换到高级难度")
                        ...
                        if new_difficulty == "beginner":
                            logger.info(f"[{self.name}] ✅ 成功切换回初级难度")
                            self.results['difficulty_selector'] = True
                        else:
                            logger.error(f"[{self.name}] ❌ 难度切换失败")
                            self.results['difficulty_selector'] = False
                    else:
                        logger.error(f"[{self.name}] ❌ 高级难度切换失败")
                        self.results['difficulty_selector'] = False
                else:
                    logger.error(f"[{self.name}] ❌ 中级难度切换失败")
                    self.results['difficulty_selector'] = False
            else:
                logger.warning(f"[{self.name}] ℹ️ 未找到难度选择器")
                self.results['difficulty_selector'] = False
            
            # 测试场景卡片加载
            await page.wait_for_timeout(3000)  # 等待场景加载
            scenario_cards_count = await page.locator('.scenario-card').count()
            logger.info(f"[{self.name}] 发现 {scenario_cards_count} 个场景卡片")
            
            if scenario_cards_count > 0:
                logger.info(f"[{self.name}] ✅ 场景卡片加载正常")
                self.results['scenario_cards'] = True
            else:
                logger.warning(f"[{self.name}] ⚠️ 未发现场景卡片")
                self.results['scenario_cards'] = False
                
        except Exception as e:
            logger.error(f"[{self.name}] 测试过程中发生错误: {str(e)}")
            self.results['navigation'] = False
            self.results['difficulty_selector'] = False
            self.results['scenario_cards'] = False
            ...
            await page.close()
            logger.info(f"[{self.name}] 测试完成")
            
        return self.results


class CalculatorExponentialAgent:
    """智能体2: 计算器和指数增长功能测试"""
    
    def __init__(self):
        self.name = "计算器指数增长智能体"
        self.results = {}
        
    async def run_test(self, browser):
        logger.info(f"[{self.name}] 开始测试计算器和指数增长功能")
            ...
            if all([principal_input, rate_input, time_input, calc_button]):
                ...
                if result_container:
                    ...
                    if result_text and "复利" in result_text:
                        logger.info(f"[{self.name}] ✅ 复利计算器功能正常")
                        self.results['compound_calculator'] = True
                    else:
                        logger.warning(f"[{self.name}] ⚠️ 复利计算器结果未显示或格式异常")
                        self.results['compound_calculator'] = False
                else:
                    logger.warning(f"[{self.name}] ⚠️ 未找到复利计算器结果容器")
                    self.results['compound_calculator'] = False
            else:
                logger.error(f"[{self.name}] ❌ 复利计算器组件缺失")
                self.results['compound_calculator'] = False
            ...
            if all([base_input, exponent_input, exp_calc_button]):
                ...
                if exp_result_container:
                    ...
                    if exp_result_text and "2^10" in exp_result_text:
                        logger.info(f"[{self.name}] ✅ 指数计算器功能正常")
                        self.results['exponential_calculator'] = True
                    else:
                        logger.warning(f"[{self.name}] ⚠️ 指数计算器结果未显示或格式异常")
                        self.results['exponential_calculator'] = False
                else:
                    logger.warning(f"[{self.name}] ⚠️ 未找到指数计算器结果容器")
                    self.results['exponential_calculator'] = False
            else:
                logger.error(f"[{self.name}] ❌ 指数计算器组件缺失")
                self.results['exponential_calculator'] = False
...
class UserInteractionGameAgent:
    """智能体3: 用户交互和游戏流程测试"""
    ...
    async def run_test(self, browser):
        logger.info(f"[{self.name}] 开始测试用户交互和游戏流程")
        ...
        try:
            ...
            if scenario_cards:
                ...
                logger.info(f"[{self.name}] 发现 {interactive_elements} 个交互元素")
                
                if interactive_elements > 0:
                    logger.info(f"[{self.name}] ✅ 场景交互功能正常")
                    self.results['interaction_elements'] = True
                    ...
                    if radio_buttons:
                        ...
                        logger.info(f"[{self.name}] ✅ 成功与单选按钮交互")
                        self.results['radio_interaction'] = True
                    else:
                        logger.info(f"[{self.name}] ℹ️ 未找到单选按钮")
                        self.results['radio_interaction'] = False
                    ...
                    if text_inputs > 0:
                        ...
                        if await text_input.count() > 0:
                            ...
                            logger.info(f"[{self.name}] ✅ 成功与文本输入框交互")
                            self.results['text_input_interaction'] = True
                        else:
                            logger.info(f"[{self.name}] ℹ️ 未找到可交互的文本输入框")
                            self.results['text_input_interaction'] = False
                    else:
                        logger.info(f"[{self.name}] ℹ️ 未找到文本输入框")
                        self.results['text_input_interaction'] = False
                    ...
                    for submit_selector in submit_selectors:
                        try:
                            ...
                            if await submit_btn.count() > 0:
                                ...
                                logger.info(f"[{self.name}] ✅ 成功点击提交按钮")
                                submitted = True
                                self.results['submit_interaction'] = True
                                break
                        except:
                            continue
                    
                    if not submitted:
                        logger.info(f"[{self.name}] ℹ️ 未找到提交按钮")
                        self.results['submit_interaction'] = False
                        
                else:
                    logger.warning(f"[{self.name}] ⚠️ 场景中未发现交互元素")
                    self.results['interaction_elements'] = False
                    self.results['radio_interaction'] = False
                    self.results['text_input_interaction'] = False
                    self.results['submit_interaction'] = False
            else:
                logger.warning(f"[{self.name}] ⚠️ 未找到场景卡片")
                self.results['interaction_elements'] = False
                self.results['radio_interaction'] = False
                self.results['text_input_interaction'] = False
                self.results['submit_interaction'] = False
...
class APIDataSyncAgent:
    """智能体4: API连接和数据同步功能测试"""
    
    def __init__(self):
        self.name = "API数据同步智能体"
        self.results = {}
        
    async def run_test(self, browser):
        logger.info(f"[{self.name}] 开始测试API连接和数据同步功能")
        ...
        try:
            ...
            # 测试API连接
            logger.info(f"[{self.name}] 测试API端点连接...")
            ...
            for api_name, result in api_results.items():
                ...
                if result.get('ok', False):
                    logger.info(f"[{self.name}] ✅ {api_name} API: 状态码 {result.get('status', 'N/A')}")
                    api_success_count += 1
                else:
                    logger.warning(f"[{self.name}] ❌ {api_name} API: {result.get('error', '连接失败')}")
            
            if api_success_count >= 2:  # 至少一半API正常
                logger.info(f"[{self.name}] ✅ API连接测试通过 ({api_success_count}/{total_apis})")
                self.results['api_connection'] = True
            else:
                logger.warning(f"[{self.name}] ⚠️ API连接测试部分失败 ({api_success_count}/{total_apis})")
            ...
            if sync_button:
                try:
                    ...
                    await page.wait_for_timeout(2000)
                    logger.info(f"[{self.name}] ✅ 数据同步按钮点击成功")
                    self.results['sync_functionality'] = True
                except Exception as e:
                    logger.warning(f"[{self.name}] ⚠️ 数据同步功能测试失败: {str(e)}")
                    self.results['sync_functionality'] = False
            else:
                logger.info(f"[{self.name}] ℹ️ 未找到数据同步按钮")
                self.results['sync_functionality'] = False
...
async def run_concurrent_tests():
    ...
    async with async_playwright() as p:
        ...
        # 创建四个测试智能体
        agent1 = ScenarioNavigationAgent()
        agent2 = CalculatorExponentialAgent()
        agent3 = UserInteractionGameAgent()
        agent4 = APIDataSyncAgent()
        
        # 并发运行所有测试
        results = await asyncio.gather(
            agent1.run_test(browser),
            agent2.run_test(browser),
            agent3.run_test(browser),
            agent4.run_test(browser)
        )
