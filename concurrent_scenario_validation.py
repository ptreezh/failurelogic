"""
Concurrent Scenario Validation Script
This script uses Playwright to concurrently validate all scenarios with interactions in Microsoft Edge browser.
...
class ScenarioValidator:
    ...
    # Removed the initialize_browser method since each scenario validation creates its own browser instance
    # This avoids resource conflicts when running concurrent validations
    ...
    async def validate_single_scenario(self, scenario: Dict) -> Dict:
        ...
        # Create a new browser instance for this scenario validation to avoid conflicts
        async with async_playwright() as p:
            # Launch Microsoft Edge browser in NON-HEADLESS MODE for this specific validation
    ...
    async def run_concurrent_validation(self):
        """Run concurrent validation of all scenarios"""
        print("🚀 Starting concurrent scenario validation...")
        print(f"📅 Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"🌐 Testing with Microsoft Edge (non-headless mode)")
        print(f"📊 Total scenarios to validate: {len(self.all_scenarios)}")

        # Create tasks for concurrent execution - each task will create its own browser instance
        tasks = []
        for scenario in self.all_scenarios:
            task = asyncio.create_task(self.validate_single_scenario(scenario))
            tasks.append(task)
        ...
        for result in results:
            if isinstance(result, Exception):
                print(f"❌ Exception during validation: {result}")
...
async def main():
    """Main function to run the concurrent scenario validation"""
    print("🚀 INITIALIZING CONCURRENT SCENARIO VALIDATION SYSTEM")
    print("="*80)
    ...
    results = await validator.run_concurrent_validation()
