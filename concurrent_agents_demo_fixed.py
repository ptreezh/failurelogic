...
async def run_playwright_test(agent_id):
    ...
    async with async_playwright() as p:
        ...
        # 访问前端应用
        print(f"🤖 子智能体 {agent_id}: 正在访问前端应用 (http://localhost:8080/minimal-complete-index.html)...")
        await page.goto("http://localhost:8080/minimal-complete-index.html")
        ...
        try:
            ...
            if await start_button.count() > 0:
                ...
                print(f"🤖 子智能体 {agent_id}: 未找到开始按钮，尝试其他选择")
                # 尝试点击导航到场景
                scenarios_link = page.locator('a[data-page="scenarios"]')
                if await scenarios_link.count() > 0:
                    await scenarios_link.click()
                    await page.wait_for_timeout(2000)
                    print(f"🤖 子智能体 {agent_id}: 已导航到场景页面")
        except Exception as e:
            print(f"🤖 子智能体 {agent_id}: 与首页交互失败 - {e}")
            # 尝试直接点击导航栏中的场景按钮
            try:
                nav_scenario_btn = page.locator('button[data-page="scenarios"]').first
                if await nav_scenario_btn.count() > 0:
                    await nav_scenario_btn.click()
                    await page.wait_for_timeout(2000)
                    print(f"🤖 子智能体 {agent_id}: 已通过导航栏点击场景")
            except Exception as e2:
                print(f"🤖 子智能体 {agent_id}: 导航到场景也失败 - {e2}")
        ...
        try:
            ...
            if await start_challenges.count() > 0:
                # 点击第一个可用的开始挑战按钮
                await start_challenges.first.click()
                await page.wait_for_timeout(3000)
                print(f"🤖 子智能体 {agent_id}: 已开始第一个场景")
                
                # 进行第一个决策
                decision_buttons = page.locator('button.decision-btn')
                if await decision_buttons.count() > 0:
                    await decision_buttons.first.click()
                    await page.wait_for_timeout(2000)
                    print(f"🤖 子智能体 {agent_id}: 已做出第一个决策")
                    
                    # 进行第二个决策（如果有）
                    if await decision_buttons.count() > 1:
                        await decision_buttons.nth(1).click()
                        await page.wait_for_timeout(2000)
                        print(f"🤖 子智能体 {agent_id}: 已做出第二个决策")
            else:
                print(f"🤖 子智能体 {agent_id}: 未找到开始挑战按钮")
        except Exception as e:
            print(f"🤖 子智能体 {agent_id}: 与场景交互失败 - {e}")
