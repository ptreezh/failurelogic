...
def verify_all_components():
    ...
    try:
        ...
        if response.status_code == 200 and ("认知" in response.text or "Failure" in response.text):
            print("✅ 前端服务正常运行")
            success_count += 1
        else:
            print("❌ 前端服务异常")
    ...
    try:
        ...
        # 检查是否包含关键术语
        has_cognitive_element = "认知" in content and ("陷阱" in content or "偏差" in content)
        has_failure_logic = "Failure Logic" in content or "failure logic" in content.lower()

        if has_cognitive_element and has_failure_logic:
            print("✅ 页面内容符合认知陷阱主题")
            success_count += 1
        else:
            print("❌ 页面内容不符合认知陷阱主题")
            print(f"   调试信息: 包含'认知'和('陷阱'或'偏差'): {has_cognitive_element}, 包含'Failure Logic': {has_failure_logic}")
            print(f"   调试信息: 包含'Failure Logic 认知陷阱': {'Failure Logic 认知陷阱' in content}")
    except Exception as e:
        print(f"❌ 无法验证页面内容: {e}")
