...
- **`release-ops`** — Versioned releases: annotated or GPG-signed tags, release notes (commits since last tag, oneline or JSON), GitHub release creation, signature verification. Wrapper: `scripts/release-ops.{ps1,sh}`. Subcommands: `notes`, `tag`, `release`, `verify`. Tagging works without tokens; `release` reuses `auth-gh` setup. Use `--sign` (PS: `-Sign`) on tag/release for GPG signing.
- **`verify-deployment-claims`** — **mandatory before claiming "deployed", "live", "ready", or "fixed"**. Always HTTP-probe the URL, distinguish 3-6s config errors from 30s+ runtime errors, and never trust CLAUDE.md over current HTTP state. See `.claude/skills/verify-deployment-claims/SKILL.md`. A PostToolUse hook in `.claude/hooks/post-deploy-reminder.py` triggers a reminder after `git push` / `npx vercel` / `vercel --prod` / `kubectl apply` / similar.
