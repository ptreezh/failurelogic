"""
最终版全面端到端测试脚本 - 针对前端服务器在8081端口的情况
使用JavaScript进行SPA导航，解决页面切换问题
"""
...
async def navigate_to_page_js(page, page_name):
    """使用JavaScript进行SPA页面导航"""
    try:
        # 使用页面内的导航功能
        result = await page.evaluate(f"""
            () => {{
                // 查找并点击对应页面的导航按钮
                const navButton = document.querySelector('button[data-page="{page_name}"]');
                if (navButton) {{
                    navButton.click();
                    return true;
                }} else {{
                    console.error('未找到页面 "{page_name}" 的导航按钮');
                    // 尝试其他可能的选择器
                    const altSelectors = [
                        `a[href="#{page_name}"]`,
                        `[data-page="{page_name}"]`,
                        `#{page_name}-button`,
                        `.nav-item[data-page="{page_name}"]`
                    ];
        ...
        print(f"❌ 使用JavaScript导航到{page_name}页面失败: {e}")
        return False
...
async def test_scenario_navigation(page):
    ...
    try:
        ...
        # 使用JavaScript进行导航
        success = await navigate_to_page_js(page, "scenarios")
        if not success:
            print("❌ JavaScript导航失败")
            return False
        
        # 检查页面是否激活
        try:
            await page.wait_for_selector("#scenarios-page.page.active", timeout=5000)
            print("✅ 成功导航到场景页面")
            return True
        except:
            print("⚠️ 未能确认到达场景页面")
...
async def test_exponential_growth_scenario(page):
    ...
    try:
        ...
        # 使用JavaScript进行导航
        success = await navigate_to_page_js(page, "exponential")
        if not success:
            print("❌ JavaScript导航失败")
            return False
        ...
        # 检查页面是否激活
        try:
            await page.wait_for_selector("#exponential-page.page.active", timeout=5000)
            print("✅ 成功导航到指数增长页面")
        except:
...
async def test_compound_interest_scenario(page):
    ...
    try:
        ...
        # 使用JavaScript进行导航
        success = await navigate_to_page_js(page, "exponential")  # 复利计算器在指数页面中
        if not success:
            print("❌ JavaScript导航失败")
            return False
...
async def test_historical_decision_scenario(page):
    ...
    try:
        ...
        # 使用JavaScript进行导航
        success = await navigate_to_page_js(page, "about")
        if not success:
            print("❌ JavaScript导航失败")
            return False
        ...
        # 检查页面是否激活
        try:
            await page.wait_for_selector("#about-page.page.active", timeout=5000)
            print("✅ 成功导航到关于页面")
        except:
            print("⚠️ 未能确认到达关于页面")
